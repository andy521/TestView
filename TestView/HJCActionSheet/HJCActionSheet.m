...
- (instancetype)initWithDelegate:(id<HJCActionSheetDelegate>)delegate CancelTitle:(NSString *)cancelTitle TitleArray:(NSMutableArray *)titleArray
{
    ...
    for (int i=0; i<titleArray.count; i++) {
        ...
        [self setupBtnImageWithTitle:dic];
    }
...
- (instancetype)initWithDelegate:(id<HJCActionSheetDelegate>)delegate CancelTitle:(NSString *)cancelTitle title:(NSString *)title OtherTitles:(NSString *)otherTitles, ...
{
    HJCActionSheet *actionSheet = [self init];
    self.actionSheet = actionSheet;
    ...
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, BtnHeight * (_tag - 1)+40, ScreenWidth, BtnHeight)];
   ...
   // [btn setBackgroundImage:highImage forState:UIControlStateHighlighted];
    ...
    // 取消按钮
    btn = [[UIButton alloc] initWithFrame:CGRectMake(0, sheetView.frame.size.height - BtnHeight, ScreenWidth, BtnHeight)];
    [btn setBackgroundImage:normalImage forState:UIControlStateNormal];
    [btn setBackgroundImage:highImage forState:UIControlStateHighlighted];
    [btn setTitle:cancelTitle forState:UIControlStateNormal];
    [btn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
...
-(void)setupBtnImageWithTitle:(NSDictionary *)dic{
    // 创建按钮
    NSString *title=[dic objectForKey:@"title"];
    UIImage *img= [UIImage  imageNamed:dic[@"img"]];
    UIImage *tapimg=[UIImage  imageNamed:dic[@"tapimg"]];
    UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(0, BtnHeight * (_tag - 1) , ScreenWidth, BtnHeight)];
    [btn setBackgroundImage:normalImage forState:UIControlStateNormal];
    [btn setBackgroundImage:highImage forState:UIControlStateHighlighted];
    [btn setImage:img forState:UIControlStateNormal];
    [btn setImage:tapimg forState:UIControlStateHighlighted];
    [btn setTitle:title forState:UIControlStateNormal];
    [btn setImageEdgeInsets:UIEdgeInsetsMake(0.0, 0, 0.0, 20.0)];
    [btn setTitleColor:[UIColor redColor ] forState:UIControlStateNormal];
   // [btn setTitleColor:[IHUtility colorWithHexString:@"#333333"] forState:UIControlStateHighlighted];
    btn.titleLabel.font = HeitiLight(17);
    btn.tag = _tag;
    [btn addTarget:self action:@selector(sheetBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    [self.sheetView addSubview:btn];

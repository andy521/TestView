...
- (void)layoutSubviews
{
    ...
    if (self.bounds.size.width * (imageSize.height / imageSize.width) > self.bounds.size.height) {
        ...
        _scrollImageView.bounds = CGRectMake(0, 0, kScreenWidth, imageViewH);
...
{
    if ((_totalScale < 1 && totalScale < _totalScale) || (_totalScale > 2.0 && totalScale > _totalScale)) return; // 最大缩放 2倍,最小0.5倍
...
- (void)prepareForImageViewScaling
{
    if (!_zoomingScroolView) {
        ...
        _zoomingScroolView.backgroundColor = [UIColor redColor];

...
- (void)viewWillAppear:(BOOL)animated {
   ...
   // [UIApplication sharedApplication].statusBarStyle = iOS7Later ? UIStatusBarStyleDefault : UIStatusBarStyleLightContent;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    ...
    if (self.childViewControllers.count > 0) {
        ...
        [backButton setImage:[UIImage imageNamed:@"nav-back.png"] forState:UIControlStateNormal];
//        backButton.imageEdgeInsets = UIEdgeInsetsMake(0, -5, 0, 0);
//        [backButton setTitle:@"返回" forState:UIControlStateNormal];
//        [backButton setTitleColor:mineTextFontColor forState:UIControlStateNormal];
...
- (void)configTableView {
    ...
    [[TZImageManager manager] getAllAlbums:imagePickerVc.allowPickingVideo allowPickingImage:imagePickerVc.allowPickingImage completion:^(NSArray<TZAlbumModel *> *models) {
        ...
        if (!_tableView) {
            CGFloat top = 44;
            if (iOS7Later) top += 20;
            _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, top+5, self.view.tz_width, self.view.tz_height - top) style:UITableViewStylePlain];
